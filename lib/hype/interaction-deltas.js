 ...
 */
function normalizeVoteValue(existingVote) {
  const value = Number(existingVote?.value);
...
export function getHypeKindsForInteraction(action, ctx = {}) {
  ...
  const voteValue = normalizeVoteValue(existingVote);

  switch (action) {
    case "upvote":
      if (voteValue === 1) return ["unlike"];
      if (voteValue === -1) return ["undownvote", "like"];
      return ["like"];

    case "downvote":
      if (voteValue === -1) return ["undownvote"];
      if (voteValue === 1) return ["unlike", "downvote"];
      return ["downvote"];
