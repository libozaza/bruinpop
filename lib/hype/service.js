 ...
 * Prompt: One atomic User update for multiple hype kinds; skip DB if net delta 0; mirror recordHostEngagement $max pipeline.
 * Solution: sumHostHypeDeltas(kinds) → single findOneAndUpdate with $max(0, hypeScore + net).
 * Reflection: Batching avoids racey double-writes on vote switches; I still own tier thresholds elsewhere in lib/hype.
 *
...
/**
 * Apply hype deltas to the post host when the actor is not the host.
 *
 * @param {Object} args
 * @param {string|import('mongoose').Types.ObjectId} args.hostId
 * @param {string|import('mongoose').Types.ObjectId} args.actorUserId
 * @param {import('./engagement').EngagementKind[]} args.kinds
 * @returns {Promise<import('./tiers').HostHypePayload | null>}
 */
export async function applyHostHypeIfNotSelf({ hostId, actorUserId, kinds }) {
  if (
    !hostId ||
    !actorUserId ||
    !Array.isArray(kinds) ||
    kinds.length === 0 ||
    String(hostId) === String(actorUserId)
  ) {
    return null;
  }
  return recordHostEngagements({ hostId, kinds });
}
