...
// Prompt: node:test every getHypeKindsForInteraction branch + sumHostHypeDeltas nets (toggle, down→up, duplicate rsvp).
// Solution: assert.deepEqual on kind arrays; strictEqual on summed deltas (+2 switch, -1 unlike).
// Reflection: AI draft caught a wrong “net zero on switch” assumption; I kept tests as the contract before merging.
...
test("upvote: coerces string vote value when toggling off", () => {
  assert.deepEqual(
    getHypeKindsForInteraction("upvote", { existingVote: { value: "1" } }),
    ["unlike"],
  );
