...
test("HOST_HYPE_DELTA: downvote is the only negative weight", () => {
  for (const [kind, delta] of Object.entries(HOST_HYPE_DELTA)) {
    if (kind === "downvote") {
      assert.ok(delta < 0, "downvote should reduce host hype");
    } else {
      assert.ok(delta >= 0, `${kind} should not reduce host hype`);
    }
  }
});
...
test("applyHostHypeDelta: floors at 0 when delta would drive negative", () => {
  // Robust to any future weights: pick a base smaller than |downvote weight|
  // and verify the floor still holds.
