...
test("postToMapEvent normalizes API post for map components", () => {
  ...
  assert.equal(event.pinCategoryId, "study");
});

test("postToMapEvent uses primary category when several tags are set", () => {
  const event = postToMapEvent({
    id: "abc123",
    title: "Mixer",
    location: { lat: 34.07, lng: -118.44 },
    categories: ["other", "party", "food"],
  });

  assert.equal(event?.pinCategoryId, "party");
});
