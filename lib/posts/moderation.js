import { getValidCategoryIds } from "./categories";

// list of report reasons (could be expanded to include custom user-defined reasons)
...
// GenAI recommended helper function to clean text for security before checking it against the basic blocked terms list
...
export function validatePostPayload({ title, content, categories }) {
  ...
  const safeCategories = getValidCategoryIds(categories);
